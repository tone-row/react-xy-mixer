...
function laloprint(x, htmlId, append) {
  ...
  if (typeof htmlId == "undefined") var htmlId = "LALOLibOutput";
  ...
  if (typeof htmlId == "undefined" || htmlId === true) {
    ...
    if (type(A) == "matrix") {
      ...
      for (i = 0; i < m; i++) {
        ...
        if (i < m - 1) str += printNumber(A.val[i * A.n + j]) + "]; ";
        else str += printNumber(A.val[i * A.n + j]) + "]";
      }
      str += "]";
      return str;
    ...
    //}
    if (append) document.getElementById(htmlId).innerHTML += html;
    else document.getElementById(htmlId).innerHTML = html;
    /*
		if ( domathjax) 
  ...
  switch (typeof x) {
    ...
    default:
      ...
      if (Math.abs(x - x_int) < 2.23e-16) {
        return "" + x_int;
      } else return x.toFixed(printPrecision);
...
function plot(multiargs) {
  ...
  if (minY > 0) minY = -0.1 * maxY;

  if (maxY < 0) maxY = -0.1 * minY;
  ...
  LALOLibOutput.innerHTML +=
    "<br><div style='position:relative;left:0px;top:0px;text-align:left;'> <div><a onmousemove='mouseposition(event," +
    LALOLibPlotsIndex +
    ");' onmousedown='mousestartmove(event," +
    LALOLibPlotsIndex +
    ");' onmouseup='mousestopmove(event);' onmouseleave='mousestopmove(event);' ondblclick='zoomoriginal(" +
    LALOLibPlotsIndex +
    ");'><canvas id='" +
    plotid +
    "'  width='500' height='500' style='border: 1px solid black;'></canvas></a></div> <label id='lblposition" +
    LALOLibPlotsIndex +
    "'></label> <div style='position: absolute;left: 550px;top: -1em;'> <canvas id='legend" +
    LALOLibPlotsIndex +
    "' width='" +
    legendwidth +
    "' height='500'></canvas></div> <div id='legendtxt" +
    LALOLibPlotsIndex +
    "' style='position: absolute;left: 610px;top: 0;'></div> </div>";
  ...
  LALOLibPlots[LALOLibPlotsIndex].setScalePlot(
    plotinfo.minX,
    plotinfo.maxX,
    200,
    plotinfo.scaleY
  ...
  if (plotinfo.minY && plotinfo.maxY) {
    LALOLibPlots[LALOLibPlotsIndex].view(
      plotinfo.minX,
      plotinfo.maxX,
      plotinfo.minY,
      plotinfo.maxY
  ...
  for (p = 0; p < plotinfo.data.length; p++) {
    ...
    if (typeof plotinfo.styles[p] == "string") {
      ...
      if (color < 0) color = colors.splice(0, 1)[0];
      // pick next unused color
      else colors.splice(colors.indexOf(color), 1); // remove this color
    } else color = color = colors.splice(0, 1)[0]; // pick next unused color

    if (typeof color == "undefined")
      // pick black if no next unused color
      color = 0;

    for (i = 0; i < plotinfo.data[p][0].length; i++) {
      if (pointstyle)
        LALOLibPlots[LALOLibPlotsIndex].addPoint(
          plotinfo.data[p][0][i],
          plotinfo.data[p][1][i],
          color
        );
      if (linestyle && i < plotinfo.data[p][0].length - 1)
        LALOLibPlots[LALOLibPlotsIndex].plot_line(
          plotinfo.data[p][0][i],
          plotinfo.data[p][1][i],
          plotinfo.data[p][0][i + 1],
          plotinfo.data[p][1][i + 1],
          color
        );
    }
    ...
    if (plotinfo.legend[p] != "") {
      var ctx = document
        .getElementById("legend" + LALOLibPlotsIndex)
        .getContext("2d");
      setcolor(ctx, color);
      ctx.lineWidth = "3";
      if (pointstyle) {
      ...
      document.getElementById("legendtxt" + LALOLibPlotsIndex).innerHTML +=
        plotinfo.legend[p] + "<br>";
    }
  }
  for (var pi = 0; pi <= LALOLibPlotsIndex; pi++) LALOLibPlots[pi].replot();

  // ZOOM
  if (window.addEventListener)
    document
      .getElementById(plotid)
      .addEventListener("DOMMouseScroll", this.mousezoom, false); //firefox
...
function colorplot(multiargs) {
  ...
  LALOLibOutput.innerHTML +=
    ...
    "'  width='500' height='500' style='border: 1px solid black;'></canvas></a></div> <label id='lblposition" +
    LALOLibPlotsIndex +
    "'></label> <div style='position: absolute;left: 550px;top: -1em;'><label id='legendmaxZ" +
    LALOLibPlotsIndex +
    "' style='font-family:verdana;font-size:80%;'></label><br>  <canvas id='legend" +
    LALOLibPlotsIndex +
    "' width='" +
    legendwidth +
    "' height='500'></canvas><br><label id='legendminZ" +
    LALOLibPlotsIndex +
    "' style='font-family:verdana;font-size:80%;'></label></div> <div id='legendtxt" +
    LALOLibPlotsIndex +
    "' style='position: absolute;left: 610px;top: 0;'></div> </div>";

  LALOLibPlots[LALOLibPlotsIndex] = new ColorPlot(plotid);
  LALOLibPlots[LALOLibPlotsIndex].setScale(
    plotinfo.minX,
    plotinfo.maxX,
    plotinfo.minY,
    plotinfo.maxY,
  ...
  if (legendcanvas) var legendheight = legendcanvas.height;
  else var legendheight = 500;

  var y;
  for (var i = 0; i < LALOLibPlots[LALOLibPlotsIndex].cmap.length; i++) {
    y = Math.floor(
      (i * legendheight) / LALOLibPlots[LALOLibPlotsIndex].cmap.length
    );
    ctx.fillStyle =
      "rgb(" +
      LALOLibPlots[LALOLibPlotsIndex].cmap[i][0] +
      "," +
      LALOLibPlots[LALOLibPlotsIndex].cmap[i][1] +
      "," +
      LALOLibPlots[LALOLibPlotsIndex].cmap[i][2] +
      ")";
    ctx.fillRect(
      0,
      legendheight - y,
      legendwidth,
      legendheight / LALOLibPlots[LALOLibPlotsIndex].cmap.length + 1
    );
  }

  document.getElementById(
    "legendmaxZ" + LALOLibPlotsIndex
  ).innerHTML = plotinfo.maxZ.toPrecision(3);
  document.getElementById(
    "legendminZ" + LALOLibPlotsIndex
  ).innerHTML = plotinfo.minZ.toPrecision(3);
...
function plot3(multiargs) {
  ...
  while (p < arguments.length) {
    if (type(arguments[p]) == "vector") {
      if (
        ...
        type(arguments[p + 1]) == "vector" &&
        type(arguments[p + 2]) == "vector"
      ) {
  ...
  LALOLibOutput.innerHTML +=
    '<br><div style="position:relative;left:0px;top:0px;text-align:left;"> <div><a onmousedown="LALOLibPlots[' +
    LALOLibPlotsIndex +
    '].mousedown(event);" onmouseup="LALOLibPlots[' +
    LALOLibPlotsIndex +
    '].mouseup(event);" onmousemove="LALOLibPlots[' +
    LALOLibPlotsIndex +
    '].mouserotation(event);"><canvas id="' +
    plotid +
    '" width="500" height="500" style="border: 1px solid black;" title="Hold down the mouse button to change the view and use the mousewheel to zoom in or out." ></canvas></a></div><label id="lblposition' +
    LALOLibPlotsIndex +
    '"></label> <div style="position: absolute;left: 550px;top: -1em;"> <canvas id="legend' +
    LALOLibPlotsIndex +
    '" width="' +
...
function image(X, title) {
  ...
  for (i = 0; i < m; i++) {
    var Xi = X.row(i);
    for (j = 0; j < n; j++) {
      // could do for j in X[i] if colormap for 0 is white...
  ...
  if (imagedata.style[0] > height) height = imagedata.style[0];
  ...
  LALOLibOutput.innerHTML +=
    '<div style="position:relative;left:0px;top:0px;text-align:left;"> <div><a onmousemove="mouseimageposition(event,' +
    LALOLibPlotsIndex +
    ');"><canvas id="' +
    plotid +
    '"  width="' +
    ...
    '" height="' +
    height +
    '" style="border: 1px solid black;"></canvas></a></div><label id="lblposition' +
    LALOLibPlotsIndex +
    '"></label> <div style="position: absolute;left: 550px;top: -1em;">' +
  ...
  if (canvas.getContext) {
    ...
    for (i = 0; i < imagedata.data.length; i++) {
      ...
      ctx.fillStyle =
        "rgb(" +
        Math.floor(255 * (1 - color[0])) +
        "," +
        Math.floor(255 * (1 - color[1])) +
        "," +
        Math.floor(255 * (1 - color[2])) +
        ")";
...
function mousezoom(e, delta, plotidx) {
  ...
  if (typeof plotidx == "undefined") var plotidx = 0;
...
function mouseposition(e, plotidx) {
  ...
  if (LALOLABPLOTMOVING) {
    ...
    var y =
      (plotcanvas.height - ymouse) / LALOLibPlots[plotidx].scaleY +
      LALOLibPlots[plotidx].minY;

    document.getElementById("lblposition" + plotidx).innerHTML =
...
function mouseimageposition(e, plotidx) {
  ...
  if (j < n) {
    var val =
      LALOLibPlots[plotidx].data[i * n + j][2][0] * (maxX - minX) + minX;
      ...
      if (removeSpaces(splitted_wrt_op[0]) != "") {
        ...
        if (removeSpaces(splitted_wrt_op[1]) != "") {
          ...
          cmd =
            unaryOpsFcts[o] +
            "(" +
            parseSplittedCommand(splitted_wrt_op[0]) +
            ")";
        }
      } else {
        ...
        operandA =
          unaryOpsFcts[o] +
          "(" +
          parseSplittedCommand(splitted_wrt_op[1]) +
          ")";
...
function parseAssignment(assignmentStr) {
  if (assignmentStr.indexOf("[") < 0) {
    ...
    var assign = removeSpaces(assignmentStr)
      .replace("=", "")
      .replace(",", "][");
    ...
    if (middle >= 0) {
      ...
      if (rowssplit.length == 2) {
        if (rowssplit[0] == "" && rowssplit[1] == "") rowsrange = "[]";
        else rowsrange = "range(" + rowssplit[0] + "," + rowssplit[1] + ")";
      ...
      if (colssplit.length == 2) {
        if (colssplit[0] == "" && colssplit[1] == "") colsrange = "[]";
        else colsrange = "range(" + colssplit[0] + "," + colssplit[1] + ")";
  ...
  while (i >= 0) {
    ...
    if (i >= 0) {
      ...
      if (j < cmd.length) {
        ...
        k = 0;
        while (k < i) {
          if (delimiters.indexOf(cmd[k]) >= 0) l = k + 1;
          k++;
        }
...
function parseCommand(cmdString) {
  ...
  if (idxComments >= 0) cmdString = cmdString.substr(0, idxComments);

  // Parse "=" sign to divide between assignement String and computeString
  var idxEqual = cmdString
    .split("==")[0]
    .split("!=")[0]
    .split(">=")[0]
    .split("<=")[0]
  ...
  if (idxEqual > 0) {
    ...
    if (
      assignmentStr.indexOf("set(") < 0 &&
      typeof self[removeSpaces(computeStr)] != "undefined"
    ) {
      //self.hasOwnProperty( removeSpaces(computeStr) ) ) { // self.hasOwnProperty does not work in Safari workers....
  ...
  while (i >= 0) {
    ...
    while (i >= 0 && startdelimiters.indexOf(cmd[i]) < 0) i--;
    if (i >= 0) {
      ...
      if (j < cmd.length) {
        ...
        parsedContent = parseSplittedCommand(
          parsedCommand.slice(map[i] + 1, map[j]).join("")
        );
        ...
        if (cmd[i] != ",") cmd[i] = " "; // except for commas that serve twice (once as start once as end)
        cmd[j] = " ";

        // map position in the original cmd to positions in the parsedCommand to track brackets
        for (k = i + 1; k < j; k++) map[k] = map[i] + 1;
        var deltamap = map[j] - map[i] - 1;
        for (k = j; k < cmd.length; k++) map[k] += 1 - deltamap;
  ...
  var FinalCommand =
    assignmentStr + parseSplittedCommand(parsedCommand.join(""));
  ...
  if (assignmentStr.substr(0, 4) == "set(") FinalCommand += " )";
...
function Lalolab(id, mllab, path) {
  ...
  if (typeof path == "undefined") var path = "http://mlweb.loria.fr/";
  else {
...
Lalolab.prototype.getObject = function (varname, callback) {
  this.exec("getObjectWithoutFunc(" + varname + ")", function (res) {
    callback(renewObject(res));
  });
};
...
function renewObject(obj) {
  ...
  switch (to) {
    ...
    default:
      ...
      if (typearray.length == 1) var newobj = new Class();
      ...
      if (typearray[1] == "SVM" || typearray[1] == "SVR") {
        newobj["kernelFunc"] = kernelFunction(
          newobj["kernel"],
          newobj["kernelpar"],
          type(newobj["SV"]) == "spmatrix" ? "spvector" : "vector"
        );
      }
      if (typearray[1] == "KernelRidgeRegression") {
        newobj["kernelFunc"] = kernelFunction(
          newobj["kernel"],
          newobj["kernelpar"],
          type(newobj["X"]) == "spmatrix" ? "spvector" : "vector"
        );
      }
...
function tic(T) {
  if (typeof TICTOCstartTime == "undefined") TICTOCstartTime = new Array();
...
function toc(T) {
  if (typeof T == "undefined") var T = 0;
  if (
    typeof TICTOCstartTime != "undefined" &&
    typeof TICTOCstartTime[T] != "undefined"
  ) {
    // Computing time
...
function printVector(x) {
  ...
  if (i == n - 1) str += (isInteger(x[i]) ? x[i] : x[i].toFixed(3)) + " ]";
  else str += "... ] (length = " + n + ")";
...
function arrayCopy(A) {
  ...
  for (var i = 0; i < A.length; i++)
    if (isScalar(A[i])) res[i] = A[i];
    //does not copy 2D Arrays...
    else res[i] = matrixCopy(A[i]);
  return res;
}
...
function reshape(A, m, n) {
  ...
  if (tA == "vector") {
    ...
    if (m * n != A.m * A.n) {
      ...
      if (n == 1) R = vectorCopy(A.val);
      else R = new Matrix(m, n, A.val);
    }
  } else error("Error in reshape(A): A is neither a vector nor a matrix.");
  return R;
}
...
function getRows(A, rowsrange) {
  ...
  if (n > 1) {
    ...
    return vectorCopy(
      A.val.subarray(rowsrange[0] * A.n, rowsrange[0] * A.n + A.n)
    );
}
function getCols(A, colsrange) {
  ...
  if (n > 1) {
    ...
    var rA = 0;
    for (var i = 0; i < m; i++) {
      for (var j = 0; j < n; j++) res.val[r + j] = A.val[rA + colsrange[j]];
...
function randn(dim1, dim2) {
  ...
  if (
    typeof dim1 == "undefined" ||
    (dim1 == 1 && typeof dim2 == "undefined") ||
    (dim1 == 1 && dim2 == 1)
  ) {
    return randnScalar();
  } else if (typeof dim2 == "undefined" || dim2 == 1) {
    res = new Float64Array(dim1);
    for (var i = 0; i < dim1; i++) res[i] = randnScalar();
...
function randnsparse(NZratio, dim1, dim2) {
  ...
  var NZ;
  if (NZratio > 1) NZ = NZratio;
  else NZ = Math.floor(NZratio * dim1 * dim2);
...
function randsparse(NZratio, dim1, dim2) {
  // Generates a sparse random matrix with NZratio * dim1*dim2 (or NZ if NZratio > 1 ) nonzeros
  if (typeof dim2 == "undefined") var dim2 = 1;
...
for (var mf in MathFunctions) {
  if (eval("typeof(Math." + MathFunctions[mf] + ")") == "function") {
    if (eval("Math." + MathFunctions[mf] + ".length") == 1) {
      ...
      eval(
        MathFunctions[mf] +
          "Vector = function (x) { return applyVector(Math." +
          MathFunctions[mf] +
          " , x );};"
      );
      ...
      eval(
        MathFunctions[mf] +
          "Matrix = function (x) { return applyMatrix(Math." +
          MathFunctions[mf] +
          " , x );};"
      );
    }
...
function apply(f, x) {
  ...
  switch (type(x)) {
    ...
    case "Complex":
      ...
      if (fc >= 0) return eval(ComplexFunctions[fc] + "Complex(x);");
      else {
    ...
    case "spvector":
      ...
      break;
    case "ComplexVector":
      if (f.name == "abs") return absComplex(x);
      else return applyComplexVector(f, x);
      break;
    case "matrix":
    ...
    case "ComplexMatrix":
      if (f.name == "abs") return absComplex(x);
      else return applyComplexMatrix(f, x);
      break;
    default:
      return "undefined";
  }
...
function applyVector(f, x) {
  ...
  for (var i = 0; i < nv; i++) res[i] = f(x[i]);
  return res;
}
function applyComplexVector(f, x) {
  const nv = x.length;
  var res = new ComplexVector(nv);
  for (var i = 0; i < nv; i++) res.set(i, f(x.get(i)));
  return res;
}
function applyComplexMatrix(f, x) {
  ...
  for (var i = 0; i < m; i++)
    for (var j = 0; j < n; j++) res.set(i, j, f(x.get(i, j)));
  return res;
}
...
function entrywisemul(a, b) {
  var sa = size(a);
  var sb = size(b);
  if (typeof a != "number" && sa[0] == 1 && sa[1] == 1) a = get(a, 0, 0);
  if (typeof b != "number" && sb[0] == 1 && sb[1] == 1) b = get(b, 0, 0);
    ...
    case "vector":
      switch (type(b)) {
        ...
        case "vector":
          if (a.length != b.length) {
            error(
              "Error in entrywisemul(a,b): a.length = " +
                a.length +
                " != " +
                b.length +
                " = b.length."
    ...
    case "matrix":
      switch (type(b)) {
        ...
        case "matrix":
          if (a.m != b.m || a.n != b.n) {
            error(
              "Error in entrywisemul(A,B): size(A) = [" +
                a.m +
                "," +
                a.n +
                "] != [" +
...
function outerprodVectors(a, b, scalar) {
  ...
  if (arguments.length == 3) {
    ...
    for (i = 0; i < m; i++) res.val.set(mulScalarVector(a[i], b), i * n);
  }
  return res;
}
function outerprod(u, v, scalar) {
  ...
  if (typeof u == "number") {
    if (typeof v == "number") {
      if (arguments.length == 2) return u * v;
      else return u * v * scalar;
    } else {
      if (arguments.length == 2)
        return new Matrix(1, v.length, mulScalarVector(u, v), true);
      else return new Matrix(1, v.length, mulScalarVector(u * scalar, v), true);
    }
  }
  if (u.length == 1) {
    if (typeof v == "number") {
      if (arguments.length == 2) return u[0] * v;
      else return u[0] * v * scalar;
    } else {
      if (arguments.length == 2)
  ...
  if (typeof v == "number") {
    if (arguments.length == 2) return mulScalarVector(v, u);
    else return mulScalarVector(scalar * v, u);
  }
  if (v.length == 1) {
    if (arguments.length == 2) return mulScalarVector(v[0], u);
    else return mulScalarVector(scalar * v[0], u);
  }

  if (arguments.length == 2) return outerprodVectors(u, v);
  else return outerprodVectors(u, v, scalar);
}
/**
 * @param {number}
 * @param {Float64Array}
...
function subScalarVector(scalar, vec) {
  const n = vec.length;
  var res = new Float64Array(n);
  for (var i = 0; i < n; i++) res[i] = scalar - vec[i];
...
function pow(a, b) {
  ...
  if (ta == "number" && tb == "number") return Math.pow(a, b);
    ...
    if (ta == "vector") {
      ...
      for (i = 0; i < a.length; i++) c[i] = Math.pow(a[i], b);
      return c;
    } else {
...
function minusVector(x) {
  ...
  for (var i = 0; i < x.length; i++) res[i] = -x[i];
  return res;
}
/**
 * @param {Matrix}
...
function minMatrixRows(A) {
  ...
  var r = n;
  for (var i = 1; i < m; i++) {
    for (j = 0; j < n; j++) if (A.val[r + j] < res[j]) res[j] = A.val[r + j];
    r += n;
  }
...
function max(a, b) {
  ...
  if (ta == "number" && tb == "number") return Math.max(a, b);
  else if (ta == "number") {
    if (tb == "vector") return maxVectorScalar(b, a);
    else return maxMatrixScalar(b, a);
  } else if (tb == "number") {
    if (ta == "vector") return maxVectorScalar(a, b);
    else {
      // MAtrix , scalar
      if (b == 1) return maxMatrixRows(a);
      // return row vector of max of columns
      else if (b == 2) return maxMatrixCols(a);
      // return column vector of max of rows
      else return maxMatrixScalar(a, b);
    }
  } else if (ta == "vector") {
    if (tb == "vector") return maxVectorVector(a, b);
    else return "undefined";
  } else {
    if (tb == "matrix") return maxMatrixMatrix(a, b);
    else return "undefined";
  }
}
/**
...
function det(A) {
  const n = A.n;
  if (A.m != n || typeof A.m == "undefined") return undefined;
  ...
  if (n == 2) {
    ...
    for (i = 0; i < n; i++) {
      var proddiag = 1;
      for (j = 0; j < n; j++) proddiag *= A.val[((i + j) % n) * n + j];

      detA += proddiag;
    }
    for (i = 0; i < n; i++) {
      var proddiag = 1;
      for (j = 0; j < n; j++) proddiag *= A.val[((i + n - 1 - j) % n) * n + j];
...
function trace(A) {
  if (type(A) == "matrix") {
    var n = A.length;
    if (A.m != n) return "undefined";
    var res = 0;
    for (var i = 0; i < n; i++) res += A.val[i * n + i];
    return res;
  } else {
    return undefined;
  }
...
function triu(A) {
  ...
  var r = 0;
  for (i = 0; i < im; i++) {
    for (j = i; j < n; j++) res.val[r + j] = A.val[r + j];
    r += n;
  }
  return res;
}
...
function tril(A) {
  ...
  var r = 0;
  for (i = 0; i < im; i++) {
    for (j = 0; j <= i; j++) res.val[r + j] = A.val[r + j];
    r += n;
  }
  if (m > im) {
    for (i = im; i < m; i++) {
      for (j = 0; j < n; j++) res.val[r + j] = A.val[r + j];
      r += n;
    }
  }
  return res;
...
function issymmetric(A) {
  ...
  for (var i = 0; i < m; i++)
    for (var j = 0; j < n; j++)
      if (A.val[i * n + j] != A.val[j * n + i]) return false;
...
function isEqual(a, b) {
  ...
  if (ta == "number" && tb != "number") return isEqual(b, a);
...
function isNotEqual(a, b) {
  ...
  if (ta == "number" && tb != "number") return isNotEqual(b, a);
  ...
  if (ta != "number" && tb == "number") {
    ...
    switch (ta) {
      ...
      case "vector":
        res = new Float64Array(a.length);
        for (i = 0; i < a.length; i++) {
          if (!isZero(get(a, i) - get(b, i))) res[i] = 1;
        }
        return res;
        break;
      case "matrix":
...
function isGreater(a, b) {
  ...
  if (ta == "number" && tb != "number") return isGreater(b, a);
...
function isGreaterOrEqual(a, b) {
  ...
  if (ta == "number" && tb != "number") return isGreaterOrEqual(b, a);
...
function isLower(a, b) {
  ...
  if (ta == "number" && tb != "number") return isLower(b, a);
...
function isLowerOrEqual(a, b) {
  ...
  if (ta == "number" && tb != "number") return isLowerOrEqual(b, a);
  ...
  switch (type(x)) {
    ...
    case "spvector":
      ...
      if (maxi < 0 && x.val.length < x.length) {
        idx = 0;
        while (x.ind.indexOf(idx) >= 0 && idx < x.length) idx++;
      }
      return idx;
      break;
    default:
...
function sort(x, decreasingOrder, returnIndexes) {
  ...
  if (typeof decreasingOrder == "undefined") var decreasingOrder = false;
  if (typeof returnIndexes == "undefined") var returnIndexes = false;
  ...
  if (returnIndexes) {
    var indexes = range(n);
    for (i = 0; i < n - 1; i++) {
      if (decreasingOrder) j = findmax(get(x, range(i, n))) + i;
      else j = findmin(get(x, range(i, n))) + i;
    ...
    var xs = vectorCopy(x);
    for (i = 0; i < n - 1; i++) {
      if (decreasingOrder) j = findmax(get(xs, range(i, n))) + i;
      else j = findmin(get(xs, range(i, n))) + i;
...
function sumMatrixCols(A) {
  ...
  var r = 0;
  for (var i = 0; i < m; i++) {
    for (var j = 0; j < A.n; j++) res[i] += A.val[r + j];
    r += A.n;
  }
  return res;
}
...
function prodMatrixCols(A) {
  ...
  for (var i = 0; i < m; i++) {
    res[i] = A.val[r];
    for (var j = 1; j < A.n; j++) res[i] *= A.val[r + j];
    r += A.n;
  }
  return res;
}
function prod(A, prodalongdimension) {
  switch (type(A)) {
    ...
    case "spvector":
      if (arguments.length == 1 || prodalongdimension == 1) {
        if (A.val.length < A.length) return 0;
        else return prodVector(A.val);
      } else return A.copy();
      break;
    case "matrix":
      if (arguments.length == 1) {
    ...
    case "spmatrix":
      if (arguments.length == 1) {
        if (A.val.length < A.m * A.n) return 0;
        else return prodVector(A.val);
      } else if (prodalongdimension == 1) {
...
function variance(A, alongdimension) {
  ...
  if (arguments.length > 1) var meanA = mean(A, alongdimension);
  else var meanA = mean(A);
  ...
  switch (type(A)) {
    ...
    case "spmatrix":
      if (typeof alongdimension == "undefined") {
        ...
        // var of columns
        var res = sub(
          entrywisediv(sum(entrywisemul(A, A), 1), A.length),
          entrywisemul(meanA, meanA)
        );
        return res;
      } else if (alongdimension == 2) {
        // sum all columns, result is column vector
        res = sub(
          entrywisediv(sum(entrywisemul(A, A), 2), A.n),
          entrywisemul(meanA, meanA)
        );
        return res;
      } else return undefined;
...
function std(A, alongdimension) {
  if (arguments.length > 1) return sqrt(variance(A, alongdimension));
  else return sqrt(variance(A));
}
...
function cov(X) {
  switch (type(X)) {
    ...
    case "matrix":
      ...
      return divMatrixScalar(
        xtx(subMatrices(X, outerprod(ones(X.m), mu))),
        X.m
      );
      break;
    case "spmatrix":
      var mu = mean(X, 1).row(0);
      return divMatrixScalar(
        xtx(subspMatrixMatrix(X, outerprod(ones(X.m), mu))),
        X.m
      );
      break;
    default:
...
function xtx(X) {
  ...
  for (var k = 0; k < d; k++) {
    var kd = k * d;
    for (var j = k; j < d; j++) C.val[j * d + k] = C.val[kd + j];
  }
  return C;
}
...
function norm1(A, sumalongdimension) {
  ...
  if (arguments.length == 1) return sum(abs(A));
  else return sum(abs(A), sumalongdimension);
}
function norminf(A, sumalongdimension) {
  // linf-norm of vectors and max-norm of matrices
  if (arguments.length == 1) return max(abs(A));
  else return max(abs(A), sumalongdimension);
}
function normp(A, p, sumalongdimension) {
  // lp-norm of vectors and matrices
  if (arguments.length == 2) return Math.pow(sum(pow(abs(A), p)), 1 / p);
  else return pow(sum(pow(abs(A), p), sumalongdimension), 1 / p);
}
...
function norm0(A, sumalongdimension, epsilonarg) {
  ...
  var epsilon = EPS;
  if (arguments.length == 3) epsilon = epsilonarg;
  ...
  switch (type(A)) {
    ...
    case "vector":
      ...
      } else return isGreater(abs(a), epsilon);
      break;
    case "spvector":
      if (arguments.length == 1 || sumalongdimension == 1) {
    ...
    case "matrix":
      if (arguments.length == 1) {
        ...
        for (i = 0; i < A.m; i++) {
          for (j = 0; j < A.n; j++)
            if (Math.abs(A[i * A.n + j]) > epsilon) res.val[j]++;
        }
        return res;
      } else if (sumalongdimension == 2) {
        // norm of rows, result is column vector
        ...
        for (i = 0; i < A.m; i++) {
          for (j = 0; j < A.n; j++)
            if (Math.abs(A[i * A.n + j]) > epsilon) res[i]++;
        }
        return res;
      } else return undefined;
      break;
      ...
      if (arguments.length == 1) {
        ...
        if (A.rowmajor) {
          for (var k = 0; k < A.val.length; k++)
            if (Math.abs(A.val[k]) > epsilon) res.val[A.cols[k]]++;
        } else {
          for (var i = 0; i < A.n; i++)
        ...
        if (A.rowmajor) {
          ...
          for (var k = 0; k < A.val.length; k++)
            if (Math.abs(A.val[k]) > epsilon) res[A.rows[k]]++;
        }
        return res;
      } else return undefined;
      break;
...
function norm0Vector(x, epsilon) {
  const n = x.length;
  var res = 0;
  for (var i = 0; i < n; i++) if (Math.abs(x[i]) > epsilon) res++;
  return res;
}
...
function backsubstitution(U, b) {
  ...
  if (!isZero(U.val[j * n + j])) x[j] = b[j] / U.val[j * n + j];
  ...
  for (j = n - 3; j >= 0; j--) {
    if (!isZero(U.val[j * n + j]))
      x[j] =
        (b[j] - dot(U.row(j).subarray(j + 1, n), x.subarray(j + 1, n))) /
        U.val[j * n + j];
  }
...
function forwardsubstitution(L, b) {
  ...
  if (!isZero(L.val[0])) x[0] = b[0] / L.val[0];

  if (!isZero(L.val[n + 1])) x[1] = (b[1] - L.val[n] * x[0]) / L.val[n + 1];

  for (j = 2; j < n; j++) {
    if (!isZero(L.val[j * n + j]))
      x[j] =
        (b[j] - dot(L.row(j).subarray(0, j), x.subarray(0, j))) /
        L.val[j * n + j];
  }
...
function solveWithQRcolumnpivotingMultipleRHS(A, B) {
  ...
  for (j = 0; j < r; j++) {
    ...
    set(
      ...
      range(j, m),
      [],
      sub(smallb, mul(mul(beta[j], V[j]), mul(transpose(V[j]), smallb)))
    );
  }
  // Solve R X = B with backsubstitution
  var X = zeros(n, m);
...
function chol(A) {
  ...
  if (A.n != n) {
    error(
      "Cannot compute the cholesky factorization: the matrix is not square."
    );
    return undefined;
  }
  const n2 = n * n;
  ...
  if (isNaN(sqrtLjj)) return undefined;
  // not positive definite
  else return new Matrix(n, n, L, true);
}
...
function ldlsymmetricpivoting(Aorig) {
  ...
  for (k = 0; k < n - 1; k++) {
    ...
    for (var i = k + 1; i < n; i++) A.val[i * n + k] /= alpha;

    set(
      A,
      range(k + 1, n),
      range(k + 1, n),
      sub(get(A, range(k + 1, n), range(k + 1, n)), outerprod(v, v, 1 / alpha))
    );
  }

  // Make it lower triangular
  for (var j = 0; j < n - 1; j++) {
    for (var k = j + 1; k < n; k++) A.val[j * n + k] = 0;
  }
  return { L: A, piv: piv };
}
/**
...
function qroriginal(A, compute_Q) {
  ...
  var m = A.length;
  var n = A.n;
  if (n > m) return "QR factorization unavailable for n > m.";
  ...
  for (j = 0; j < n - 1; j++) {
    ...
    set(
      ...
      subMatrices(
        ...
        outerprodVectors(
          householder.v,
          mulMatrixVector(transposeMatrix(smallR), householder.v),
          householder.beta
        )
      )
    );
  ...
  if (m > n) {
    ...
    set(
      ...
      subVectors(
        smallR,
        mulScalarVector(
          dot(householder.v, smallR) * householder.beta,
          householder.v
        )
      )
    );
      ...
      if (r > 1) {
        if (j == r - 1)
          set(
            ...
            subVectors(
              smallQ,
              mulScalarVector(dot(smallQ, V[j]) * beta[j], V[j])
            )
          );
        else
          set(
            ...
            range(j, m),
            range(j, r),
            sub(smallQ, outerprod(V[j], mul(transpose(smallQ), V[j]), beta[j]))
          );
      } else
        Q = subVectors(
          smallQ,
  ...
  while (tau > EPS && r < n - 1 && normR22 > TOL * normA) {
    ...
    if (r < n - 1) {
      ...
      set(
        ...
        sub(
          smallR,
          mul(householder.beta * mul(smallR, householder.v), householder.v)
        )
      );
    }

    // Store householder vectors and beta
    if (m > r + 1) V[r] = vectorCopy(householder.v);
    beta[r] = householder.beta;
...
function solvecg(A, b) {
  if (A.type == "spmatrix") return spsolvecg(A, b);
  else return solvecgdense(A, b);
}
...
*/
  ...
  var updateP = function (tau, r) {
    for (var i = 0; i < m; i++) p[i] = r[i] + tau * p[i];
  };
 ...
 */
function cgnr(A, b) {
  if (A.type == "spmatrix") return spcgnr(A, b);
  else return cgnrdense(A, b);
}
/** Conjugate gradient normal equation residual method for solving the rectangular system Ax = b
 * @param{Matrix}
 * @param{Float64Array}
...
*/
  ...
  var updateP = function (tau, z) {
    for (var i = 0; i < m; i++) p[i] = z[i] + tau * p[i];
  };
...
function tridiagonalize(A, returnQ) {
  ...
  for (k = 0; k < n - 2; k++) {
    ...
    p = mulScalarVector(
      householder.beta,
      mulMatrixVector(Tkp1kp1, householder.v)
    );
    w = subVectors(
      p,
      mulScalarVector(
        0.5 * householder.beta * dot(p, householder.v),
        householder.v
      )
    );
    ...
    set(
      ...
      range(k + 1, n),
      range(k + 1, n),
      subMatrices(subMatrices(Tkp1kp1, vwT), transpose(vwT))
    );
  ...
  if (returnQ) {
    var updateQ = function (j, v, b) {
      ...
      for (i = 0; i < n_j; i++) {
        var Qi = (i + j) * n + j;
        for (k = 0; k < n_j; k++) Qtv[k] += v[i] * Q.val[Qi + k];
      }
      for (i = 0; i < n_j; i++) {
        var Qi = (i + j) * n + j;
        var betavk = b * v[i];
...
function implicitSymQRWilkinsonShift(T, computeZ) {
  ...
  const mu =
    T.val[rn1 + n - 1] - t2 / (d + Math.sign(d) * Math.sqrt(d * d + t2));
  var x = T.val[0] - mu; // T[0][0]
  var z = T.val[n]; // T[1][0]
  var cs;
  if (computeZ) var csArray = new Array(n - 1);
  //var Z = eye(n);
  ...
  for (k = 0; k < n - 1; k++) {
    ...
    if (computeZ) csArray[k] = [cs[0], cs[1]];
...
function eig(A, computeEigenvectors) {
  ...
  do {
    for (i = 0; i < n - 1; i++) {
      if (
        Math.abs(D.val[i * n + i + 1]) <
        TOL *
          (Math.abs(D.val[i * n + i]) + Math.abs(D.val[(i + 1) * n + i + 1]))
      ) {
        D.val[i * n + i + 1] = 0;
        D.val[(i + 1) * n + i] = 0;
      }
    ...
    if (
      !isZero(D.val[(n - 1) * n + n - 2]) ||
      !isZero(D.val[(n - 2) * n + n - 1])
    )
      q = 0;
    else {
      q = 1;
      while (
        q < n - 1 &&
        isZero(D.val[(n - q - 1) * n + n - q - 2]) &&
        isZero(D.val[(n - q - 2) * n + n - q - 1])
      )
        q++;
      if (q >= n - 1) q = n;
    }
    ...
    do {
      ...
      while (k < n - q - 1 && zerosOnSubdiagonal == false) {
        if (isZero(D.val[(k + 1) * n + k])) zerosOnSubdiagonal = true;
        k++;
      }
    ...
    if (q < n) {
      if (computeEigenvectors) {
        var res = implicitSymQRWilkinsonShift(
          get(D, range(p, n - q), range(p, n - q)),
          true
        );
...
function eigs(A, r, smallest) {
  ...
  if (typeof r == "undefined") var r = 1;
  if (
    typeof smallest == "undefined" ||
    smallest == false ||
    smallest != "smallest"
  ) {
    if (r == 1) return eig_powerIteration(A);
    else return eig_orthogonalIteration(A, r);
  } else {
    // look for smallest eigenvalues
    if (r == 1) return eig_inverseIteration(A, 0);
    else return eig_bisect(A, r);
...
function eig_powerIteration(A, u0) {
  ...
  var u;
  if (typeof u0 == "undefined") u = randn(n);
  else u = u0;
...
function eig_orthogonalIteration(A, r) {
  ...
  for (k = 0; k < maxIters; k++) {
    ...
    if (Math.floor(k / 50) == k / 50) {
      ...
      if (norm(subMatrices(Z, mulMatrixMatrix(Q, diag(diag(V))))) < TOL) break;
    }
...
function eig_inverseIteration(A, lambda) {
  ...
  if (typeof maxIters == "undefined") var maxIters = 100;
...
function eig_inverseOrthogonalIteration(A, r) {
  if (r == 1) return eig_inverseIteration(A);
  ...
  for (k = 0; k < maxIters; k++) {
    ...
    if (Math.floor(k / 50) == k / 50) {
      ...
      if (norm(subMatrices(Z, mulMatrixMatrix(Q, V))) < TOL) break;
    }
...
*/
  ...
  var b = zeros(n);
  var i;
  for (i = 0; i < n - 1; i++) b[i] = T.val[i * n + i + 1];
  ...
  var polyq = function (x, a, b, n) {
    ...
    var qi;
    var signchanges = 0;
    if (qi_1 < EPS) signchanges = 1;

    var i;
    for (i = 1; i < n; i++) {
      qi = a[i] - x - (b[i - 1] * b[i - 1]) / qi_1;

      if (qi < EPS) signchanges++;

      if (Math.abs(qi) < EPS) qi_1 = EPS;
      else qi_1 = qi;
    }
    return signchanges;
  };
  ...
  for (var k = 1; k <= K; k++) {
    ...
    while (Math.abs(z - y) > TOL * (Math.abs(y) + Math.abs(z))) {
      ...
      if (n_lowerthan_x >= k) z = x;
      // enough eigenvalues below x, decrease upper bound to x
      else y = x; // not enough ev below x, increase lower bound to x

      // update boudns on other lambdas
      for (var j = k + 1; j <= K; j++) if (n_lowerthan_x >= j) xu[j - 1] = x;
    }
    lambda[k - 1] = (y + z) / 2;
  }
  //return lambda;
...
function bidiagonalize(A, computeU, thinU, computeV) {
  ...
  var updateB1 = function (j, v, beta) {
    ...
    for (i = 0; i < m_j; i++) {
      var Bi = (i + j) * n + j;
      for (k = 0; k < n_j; k++) Btv[k] += v[i] * B.val[Bi + k];
    }
    for (i = 0; i < m_j; i++) {
      var betavk = beta * v[i];
      var Bi = (i + j) * n + j;
  ...
  var updateB2 = function (j, v, beta) {
    ...
    for (i = j; i < m; i++) {
      var Bi = i * n + j + 1;
      var Bv = 0;
      for (k = 0; k < n_j_1; k++) Bv += B.val[Bi + k] * v[k];
      var betaBvk = beta * Bv;
      for (k = 0; k < n_j_1; k++) {
  ...
  if (computeV) {
    var updateV = function (j, v, beta) {
      ...
      for (i = 0; i < n; i++) {
        var Vi = i * n + j + 1;
        var Vv = 0;
        for (k = 0; k < n_j_1; k++) Vv += V.val[Vi + k] * v[k];
        var betaVvk = beta * Vv;
        for (k = 0; k < n_j_1; k++) {
  ...
  if (computeU) {
    ...
    var updateU = function (j, v, beta) {
      ...
      for (i = j; i < m; i++) {
        var Ui = i * nU;
        var i_j = i - j;
        for (k = 0; k < nU; k++) vtU[k] += v[i_j] * U.val[Ui + k];
      }
      for (i = j; i < m; i++) {
  ...
  if (computeU && computeV) {
    return { U: U, V: V, B: B };
  } else if (computeV) return { V: V, B: B };
  else if (computeU) return { U: U, B: B };
  else return B;
}
...
function GolubKahanSVDstep(B, i, j, m, n, computeUV) {
  ...
  var fm_1;
  if (n > 2) fm_1 = B.val[(i + n - 3) * B.n + j + n - 2];
  else fm_1 = 0;
  ...
  const mu =
    dn * dn + fm * fm - t2 / (d + Math.sign(d) * Math.sqrt(d * d + t2));
  ...
  if (computeUV) return { csU: csU, csV: csV };
}
...
*/
  ...
  if (typeof computeUV != "undefined" && computeUV !== false) {
    if (computeUV === "full") {
      ...
      if (computeUV.indexOf("U") >= 0) computeU = true;
      if (computeUV.indexOf("V") >= 0) computeV = true;
      if (computeUV.indexOf("thin") >= 0) thinU = true;
    }
    var UBV;
    if (Atransposed) {
      var tmp = computeU;
    ...
    } else UBV = bidiagonalize(A, computeU, thinU, computeV);

    if (computeU) {
      var U = transpose(UBV.U); //Utrans
    } else var U = undefined;

    if (computeV) {
      var V = UBV.V;
      var Vt = transposeMatrix(V);
    } else var V = undefined;

    var B = UBV.B;
  } else {
    if (Atransposed) var B = bidiagonalize(At, false, false, false);
    else var B = bidiagonalize(matrixCopy(A), false, false, false);
  }
  ...
  do {
    for (i = 0; i < n - 1; i++) {
      if (
        Math.abs(B.val[i * B.n + i + 1]) <
        TOL *
          (Math.abs(B.val[i * B.n + i]) +
            Math.abs(B.val[(i + 1) * B.n + i + 1]))
      ) {
        B.val[i * B.n + i + 1] = 0;
      }
    }
    ...
    while (
      q < n &&
      Math.abs(B.val[(n - q - 1) * B.n + n - q - 2]) < TOL &&
      Math.abs(B.val[(n - q - 2) * B.n + n - q - 1]) < TOL
    ) {
      q++;
    }
    if (q == n - 1) q = n;
    ...
    while (
      p < n - q &&
      Math.abs(B.val[p * B.n + p + 1]) <
        TOL *
          (Math.abs(B.val[p * B.n + p]) +
            Math.abs(B.val[(p + 1) * B.n + (p + 1)]))
    ) {
      p++;
    }
      ...
      if (DiagonalofB22isZero >= 0) {
        if (DiagonalofB22isZero < n - q - 1) {
          ...
          for (k = DiagonalofB22isZero + 1; k < n; k++) {
            cs = givens(
              B.val[k * B.n + k],
              B.val[DiagonalofB22isZero * B.n + k]
            );
            premulGivens(cs[0], cs[1], k, DiagonalofB22isZero, B);
            if (computeU) premulGivens(cs[0], cs[1], k, DiagonalofB22isZero, U);
          }
        } else {
          ...
          for (k = n - q - 2; k >= p; k--) {
            ...
            if (computeV) premulGivens(cs[0], cs[1], k, n - q - 1, Vt);
  ...
  if (computeUV) {
    if (computeV) V = transposeMatrix(Vt);
    ...
    for (i = 0; i < n; i++) {
      if (s[i] < 0) {
        if (computeV) set(V, [], i, minus(get(V, [], i)));
        s[i] = -s[i];
      }
    }
    ...
    if (computeV) V = get(V, [], indexes);
    if (computeU) {
      if (!thinU) {
        for (i = n; i < m; i++) indexes.push(i);
      }
      U = get(U, indexes, []);
    }

    if (thinU) var S = diag(s);
    else var S = mat([diag(s), zeros(m - n, n)], true);

    var Ut = undefined;
    if (computeU) Ut = transpose(U);

    if (Atransposed) {
      if (thinU) return { U: V, S: S, V: Ut, s: s };
      else return { U: V, S: transpose(S), V: Ut, s: s };
    } else {
      return { U: Ut, S: S, V: V, s: s };
    }
  } else return sort(abs(diag(B)), true);
}
...
function rank(A) {
  ...
  var rank = 0;
  var i;
  for (i = 0; i < s.length; i++) if (s[i] > 1e-10) rank++;
...
function nullspace(A) {
  ...
  var rank = 0;
  const TOL = 1e-8;
  while (rank < n && s.s[rank] > TOL) rank++;

  if (rank < n) return get(s.V, [], range(rank, n));
  else return zeros(n);
}
...
function spVector(n, values, indexes) {
  ...
  if (arguments.length <= 2) {
    if (arguments.length == 1) var nnz = n;
    // too large but more efficient at some point...
    else var nnz = values;
...
function spMatrix(m, n, values, cols, rows) {
  ...
  if (arguments.length <= 3) {
    if (arguments.length == 2) var nnz = m * n;
    // too large but more efficient at some point...
    else var nnz = values;
...
spMatrix.prototype.toRowmajor = function () {
  if (this.rowmajor) return this.copy();
  else {
...
spMatrix.prototype.row = function (i) {
  if (this.rowmajor) {
    return new spVector(
      this.n,
      this.val.subarray(this.rows[i], this.rows[i + 1]),
      this.cols.subarray(this.rows[i], this.rows[i + 1])
    );
    /*
		var s = this.rows[i];
		var e = this.rows[i+1];
    ...
    error(
      "Cannot extract sparse column from a sparse matrix in row major format."
    );
    return undefined;
  }
};
...
spMatrix.prototype.col = function (j) {
  if (!this.rowmajor)
    return new spVector(
      this.m,
      this.val.subarray(this.cols[j], this.cols[j + 1]),
      this.rows.subarray(this.cols[j], this.cols[j + 1])
    );
  else {
    error(
      "Cannot extract sparse column from a sparse matrix in row major format."
...
function fullVector(x) {
  ...
  for (k = 0; k < nnz; k++) a[x.ind[k]] = x.val[k];
...
function sparseMatrixRowMajor(A) {
  ...
  for (i = 1; i < m; i++) rows[i + 1] += rows[i];

  return new spMatrix(m, n, val, cols, rows);
}

function sparse(A, rowmajor) {
  if (typeof rowmajor == "undefined") var rowmajor = true;
  ...
  switch (type(A)) {
    case "vector":
      ...
      break;
    case "matrix":
      if (rowmajor) return sparseMatrixRowMajor(A);
      else return sparseMatrix(A);
      break;
    case "spvector":
    case "spmatrix":
      return A.copy();
...
function spdiag(val) {
  ...
  var tv = type(val);
  if (tv == "vector") return new spMatrix(n, n, val, cols, rows);
  else {
...
function spmat(elems, rowwise) {
  ...
  if (typeof rowwise == "undefined") var rowwise = true;
  ...
  if (rowwise) {
    ...
    for (k = 0; k < elems.length; k++) {
      switch (elemtypes[k]) {
        ...
        case "spmatrix":
          for (var r = 0; r < elems[k].m; r++) res.push(elems[k].row(r));
          res.push(elems[k]);
          n = elems[k].length;
          m += 1;
          nnz += elems[k].val.length;
    ...
    error(
      "spmat(..., false) for columnwise concatenation of sparse vectors not yet implemented"
    );
...
function spdot(a, b) {
  ...
  while (ka < nnza && kb < nnzb) {
    var i = a.ind[ka];
    while (b.ind[kb] < i && kb < nnzb) kb++;
    if (b.ind[kb] == i) c += a.val[ka] * b.val[kb];
    ka++;
  }
  return c;
}
...
function dotspVectorVector(a, b) {
  const nnza = a.val.length;
  var c = 0;
  for (var ka = 0; ka < nnza; ka++) c += a.val[ka] * b[a.ind[ka]];
...
function mulMatrixspVector(A, b) {
  ...
  var ri = 0;
  for (var i = 0; i < n; i++) {
    for (var k = 0; k < nnz; k++) c[i] += A.val[ri + b.ind[k]] * b.val[k];
    ri += n;
  }
  return c;
}
  ...
  if (A.rowmajor) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      for (var ka = s; ka < e; ka++) {
        var j = A.rows[ka];
        while (b.ind[kb] < j && kb < nnzb) kb++;
        if (b.ind[kb] == i) c[i] += A.val[ka] * b.val[kb];
      }
    }
  }
  return c;
...
function entrywisemulspMatrices(A, B) {
  if (A.rowmajor) {
    if (B.rowmajor) {
      ...
      for (i = 1; i < A.m; i++) rows[i + 1] += rows[i];
    ...
    if (B.rowmajor) {
      ...
      for (j = 1; j < A.n; j++) cols[j + 1] += cols[j];
...
function addScalarspVector(a, b) {
  ...
  var k;
  for (k = 0; k < n; k++) c[k] = a;
  for (k = 0; k < nnzb; k++) c[b.ind[k]] += b.val[k];
...
function addVectorspVector(a, b) {
  ...
  const n = b.length;
  var c = new Float64Array(a);
  for (var k = 0; k < nnzb; k++) c[b.ind[k]] += b.val[k];
...
function addspVectors(a, b) {
  ...
  var c = zeros(a.length);
  var k;
  for (k = 0; k < nnza; k++) c[a.ind[k]] = a.val[k];
  for (k = 0; k < nnzb; k++) c[b.ind[k]] += b.val[k];
...
function addScalarspMatrix(a, B) {
  ...
  var C = zeros(m, n);
  var i;
  for (i = 0; i < mn; i++) C.val[i] = a;
  if (B.rowmajor) {
    var ri = 0;
    for (i = 0; i < m; i++) {
      var s = B.rows[i];
      var e = B.rows[i + 1];
      for (var k = s; k < e; k++) C.val[ri + B.cols[k]] += B.val[k];
      ri += n;
    }
  } else {
    for (i = 0; i < n; i++) {
      var s = B.cols[i];
      var e = B.cols[i + 1];
      for (var k = s; k < e; k++) C.val[B.rows[k] * n + i] += B.val[k];
    }
  }
  return C;
}
...
function spsaxpy(a, x, y) {
  const nnz = x.val.length;
  for (var k = 0; k < nnz; k++) y[x.ind[k]] += a * x.val[k];
}
...
function applyspVector(f, x) {
  ...
  var i;
  const f0 = f(0);
  for (i = 0; i < n; i++) res[i] = f0;
  for (i = 0; i < nnz; i++) res[x.ind[i]] = f(x.val[i]);
  return res;
}
/**
 * @param{function}
...
function applyspMatrix(f, X) {
  ...
  if (X.rowmajor) {
    ...
    for (i = 0; i < m; i++) {
      var s = X.rows[i];
      var e = X.rows[i + 1];
      for (var k = s; k < e; k++) C.val[ri + X.cols[k]] = f(X.val[k]);
      ri += n;
    }
  } else {
    for (i = 0; i < n; i++) {
      var s = X.cols[i];
      var e = X.cols[i + 1];
      for (var k = s; k < e; k++) C.val[X.rows[k] * n + i] += f(X.val[k]);
    }
  }
  return C;
}
...
function sumspMatrixRows(A) {
  var res = zeros(A.n);
  if (A.rowmajor) {
    for (var k = 0; k < A.val.length; k++) res[A.cols[k]] += A.val[k];
  } else {
    for (var i = 0; i < A.n; i++) res[i] = sumspVector(A.col(i));
  }
...
function sumspMatrixCols(A) {
  var res = zeros(A.m);
  if (A.rowmajor) {
    for (var i = 0; i < A.m; i++) res[i] = sumspVector(A.row(i));
  } else {
    for (var k = 0; k < A.val.length; k++) res[A.rows[k]] += A.val[k];
  }
  return res;
}
/**
...
function prodspMatrixRows(A) {
  if (A.rowmajor) {
    ...
    for (var i = 0; i < A.m; i++) {
      ...
      var e = A.rows[i + 1];
      for (var j = 0; j < A.n; j++)
        if (A.cols.subarray(s, e).indexOf(j) < 0) res[j] = 0;
      for (var k = s; k < e; k++) res[A.cols[k]] *= A.val[k];
    }
  } else {
    var res = zeros(A.n);
    for (var i = 0; i < A.n; i++) {
      var a = A.col(i);
      if (a.val.length == a.length) res[i] = prodVector(a.val);
    }
  }
  return new Matrix(1, A.n, res, true);
}
...
function prodspMatrixCols(A) {
  if (A.rowmajor) {
    ...
    for (var j = 0; j < A.n; j++) {
      ...
      var e = A.cols[j + 1];
      for (var i = 0; i < A.m; i++)
        if (A.rows.subarray(s, e).indexOf(i) < 0) res[i] = 0;
      for (var k = s; k < e; k++) res[A.rows[k]] *= A.val[k];
    }
  }
  return res;
}
...
x=minl0(A,A*r)
  ...
  if (typeof M == "undefined") var M = 10;
  ...
  var x = entrywisemulVector(
    getSubVector(lpsol, range(n)),
    getSubVector(lpsol, range(n, 2 * n))
  );
...
function qp(Q, c, A, b, Aeq, beq, lb, ub, x0, epsilon) {
  ...
  /*
	...
	*/
  if (typeof epsilon === "undefined") var epsilon = 1e-3;
  ...
  if (typeof x0 === "undefined") {
    ...
    if (typeof x == "string") return "infeasible";
  } else {
    x = vectorCopy(x0);
  }
...
*/
  ...
  if (n == 1) return secant(f, grad, x);
  else if (n > 500) return steepestdescent(f, grad, x);
  else return bfgs(f, grad, x);
}
...
function steepestdescent(f, grad, x0) {
  ...
  do {
    ...
    normg > TOLgrad &&
    prevobj - obj > TOLobj &&
    norm(subVectors(x, xprev)) > TOLx
  );
  console.log(
    " OBJ: " + obj + ", norm(grad): " + normg,
    "prevobj - obj",
    prevobj - obj,
    "iter: ",
    iter
...
function bfgs(f, grad, x0) {
  ...
  do {
    ...
    dH = subMatrices(
      outerprodVectors(delta_, delta, 1 + dot(gamma, Hgamma) / deltagamma),
      addMatrices(deltagammaH, transposeMatrix(deltagammaH))
    );
    //--
...
(function (exports) {
  ...
  var Ld = (exports.mpl_initialize = function () {
    ...
    Mn = "January February March April May June July August September October November December".split(
      " "
    );
  function Nn(a) {
    for (var b = ""; 0 < a; ) (b += "^"), a--;
...
function mvnrnd(mu, Sigma, N) {
  if (arguments.length < 3) var N = 1;

  var X = randn(N, mu.length);

  if (issymmetric(Sigma)) var L = chol(Sigma);
  else var L = Sigma; // L directly provided instead of Sigma
...
function Distribution(distrib, arg1, arg2) {
  ...
  if (typeof distrib == "string") distrib = eval(distrib);
  ...
  if (distrib.prototype.pmf) this.pmf = distrib.prototype.pmf;

  if (distrib.prototype.logpdf) this.logpdf = distrib.prototype.logpdf;
  else
...
Distribution.prototype.info = function () {
  ...
  for (i in this) {
    switch (type(this[i])) {
      ...
      case "matrix":
        str +=
          i + ": matrix of size " + this[i].m + "-by-" + this[i].n + "<br>";
        break;
      case "function":
        Functions.push(i);
        break;
...
Uniform.prototype.construct = function (a, b) {
  ...
  if (typeof a == "undefined") {
    ...
    if (typeof b == "undefined") {
      ...
      if (typeof a == "number") this.values = range(a);
      else this.values = a;
      this.dimension = 1;
...
Uniform.prototype.pdf = function (x) {
  ...
  if (this.isDiscrete) {
    ...
    if (tx == "number") {
      ...
      p = zeros(x.length);
      for (var i = 0; i < x.length; i++) p[i] = pdfscalar(x[i], this.values);
    } else if (tx == "matrix") {
      p = zeros(x.m, x.n);
    ...
    if (tx == "number") {
      if (this.dimension == 1) p = pdfscalar(x, this.a, this.b, this.px);
    } else if (tx == "vector") {
      if (this.dimension == 1) {
        p = zeros(x.length);
...
Uniform.prototype.sample = function (N) {
  ...
  if (this.isDiscrete) {
    ...
    if (this.dimension == 1)
      ...
      return add(
        entrywisemul(
          outerprod(ones(N), sub(this.b, this.a)),
          rand(N, this.dimension)
        ),
        outerprod(ones(N), this.a)
      );
    }
  }
};
...
Uniform.prototype.estimate = function (X) {
  ...
  if (tX == "matrix") var x = X.val;
  else var x = X;

  var i = 0;
  while (i < x.length && Math.round(x[i]) == x[i]) i++;
  if (i < x.length) this.isDiscrete = false;
  else this.isDiscrete = true;

  // Estimate
  if (this.isDiscrete) {
    for (i = 0; i < x.length; i++) {
      var xi = Math.round(x[i]);
      if (this.values.indexOf(xi) < 0) this.values.push(xi);
    }
    this.dimension = 1;
    this.mean = (min(this.values) + max(this.values)) / 2;
    this.variance = (this.values.length * this.values.length - 1) / 12;
...
Gaussian.prototype.construct = function (mean, variance) {
  // Read params and create the required fields for a specific algorithm
  if (typeof mean == "undefined") var mu = 1;
  else if (type(mean) == "matrix") var mu = mean.val;
  else var mu = mean;

  var dim = size(mu, 1);

  if (typeof variance == "undefined") {
    if (dim == 1) var variance = 1;
    else var variance = ones(dim);
  }
...
Gaussian.prototype.pdf = function (x) {
  ...
  if (this.dimension == 1) {
    if (typeof x == "number") {
      ...
      return (
        Math.exp((-diff * diff) / (2 * this.variance)) /
        (this.std * Math.sqrt(2 * Math.PI))
      );
    } else {
      var diff = sub(x, this.mean);
      return entrywisediv(
        exp(entrywisediv(entrywisemul(diff, diff), -2 * this.variance)),
        this.std * Math.sqrt(2 * Math.PI)
      );
    }
  } else {
    if (type(x) == "vector") {
      if (x.length != this.dimension) {
        error(
          "Error in Gaussian.pdf(x): x.length = " +
            x.length +
            " != " +
            this.dimension +
            " = Gaussian.dimension."
      ...
      return (
        Math.exp(u) /
        (Math.pow(2 * Math.PI, 0.5 * this.dimension) *
          Math.sqrt(prodVector(this.variance)))
      );
    } else {
      if (x.n != this.dimension) {
        error(
          "Error in Gaussian.pdf(X): X.n = " +
            x.n +
            " != " +
            this.dimension +
            " = Gaussian.dimension."
...
Gaussian.prototype.sample = function (N) {
  ...
  if (N == 1)
    ...
    var N1 = ones(N);
    var X = add(
      entrywisemul(outerprod(N1, this.std), randn(N, this.dimension)),
      outerprod(N1, this.mean)
    );
  }
  return X;
};
...
mvGaussian.prototype.construct = function (mean, covariance) {
  ...
  if (typeof covariance == "undefined") {
    if (dim == 1) var covariance = 1;
    else var covariance = eye(dim);
  }
  ...
  if (typeof this.L == "undefined")
    error(
      "Error in new Distribution (mvGaussian, mu, Sigma): Sigma is not positive definite"
    );
...
mvGaussian.prototype.pdf = function (x) {
  ...
  if (this.dimension == 1) {
    if (typeof x == "number") {
      ...
      return (
        Math.exp((-diff * diff) / (2 * this.variance)) /
        Math.sqrt(2 * this.variance * Math.PI)
      );
    } else {
      var diff = sub(x, this.mean);
      return entrywisediv(
    ...
    if (type(x) == "vector") {
      if (x.length != this.dimension) {
        error(
          "Error in mvGaussian.pdf(x): x.length = " +
            x.length +
            " != " +
            this.dimension +
            " = mvGaussian.dimension."
        );
        return undefined;
      }
      var diff = subVectors(x, this.mean);
      ...
      return (
        Math.exp(u) /
        Math.sqrt(Math.pow(2 * Math.PI, this.dimension) * this.det)
      );
    } else {
      if (x.n != this.dimension) {
        error(
      ...
      var denominator = Math.sqrt(
        Math.pow(2 * Math.PI, this.dimension) * this.det
      );
      for (var i = 0; i < x.m; i++) {
        var diff = subVectors(x.row(i), this.mean);
        var u = -0.5 * dot(diff, cholsolve(this.L, diff));
...
mvGaussian.prototype.sample = function (N) {
  ...
  var X = add(
    mul(randn(N, this.dimension), transpose(this.L)),
    outerprod(ones(N), this.mean)
  );
...
mvGaussian.prototype.estimate = function (X) {
  ...
  if (type(X) == "matrix") {
    ...
    if (typeof this.L == "undefined")
      error(
        "Error in mvGaussian.estimate(X): covariance estimate is not positive definite"
      );
...
Bernoulli.prototype.construct = function (mean) {
  // Read params and create the required fields for a specific algorithm
  if (typeof mean == "undefined") var mean = 0.5;
...
Bernoulli.prototype.pdf = Bernoulli.prototype.pmf = function (x) {
  ...
  if (this.dimension == 1) {
    ...
    switch (tx) {
      ...
      case "spvector":
        var p = 1;
        for (var j = 0; j < x.ind[0]; j++) p *= 1 - this.mean[j];
      ...
      case "spmatrix":
        ...
        for (var i = 0; i < x.m; i++) {
          ...
          for (var j = 0; j < xr.ind[0]; j++) p[i] *= 1 - this.mean[j];
...
Bernoulli.prototype.logpdf = Bernoulli.prototype.logpmf = function (x) {
  ...
  var logpdfscalar = function (s, mu) {
    if (s == 1) return Math.log(mu);
    else if (s == 0) return Math.log(1 - mu);
  ...
  if (this.dimension == 1) {
    ...
    switch (tx) {
      ...
      case "spvector":
        var p = 0;
        for (var j = 0; j < x.ind[0]; j++) p += Math.log(1 - this.mean[j]);
        for (var k = 0; k < x.val.length - 1; k++) {
...
Bernoulli.prototype.estimate = function (X) {
  ...
  switch (type(X)) {
    ...
    default:
      error(
        "Error in Bernoulli.estimate( X ): X must be a (sp)matrix or (sp)vector."
      );
      break;
  }
  return this;
...
Poisson.prototype.construct = function (mean) {
  // Read params and create the required fields for a specific algorithm
  if (typeof mean == "undefined") var mean = 5;
...
Poisson.prototype.pdf = Poisson.prototype.pmf = function (x) {
  ...
  var pdfscalar = function (s, lambda) {
    if (s < 0 || Math.round(s) != s) return 0;
    else if (s == 0) return 1;
    else {
      var u = lambda;
      for (var k = 2; k <= s; k++) u *= lambda / k;
...
Poisson.prototype.sample = function (N) {
  ...
  if (typeof N == "undefined" || N == 1) {
    if (this.dimension == 1) return samplescalar(this.mean);
    else {
      var s = zeros(this.dimension);
      for (k = 0; k < this.dimension; k++) s[k] = samplescalar(this.mean[k]);
      return s;
    }
  } else {
    if (this.dimension == 1) {
      var S = zeros(N);
      for (var i = 0; i < N; i++) S[i] = samplescalar(this.mean);
      return S;
    } else {
...
function conj(z) {
  if (z instanceof Complex) return new Complex(z.re, -z.im);
  else if (z instanceof ComplexVector) {
    var r = new ComplexVector(z);
    for (var i = 0; i < z.length; i++) r.im[i] = -r.im[i];
    return r;
  } else if (z instanceof ComplexMatrix) {
    var r = new ComplexMatrix(z);
    for (var i = 0; i < z.length; i++) r.im[i] = -r.im[i];
    return r;
  } else return new Complex(z); // for a real
}

function modulus(z) {
  if (z instanceof Complex) return Math.sqrt(z.re * z.re + z.im * z.im);
  else if (z instanceof ComplexVector)
    return sqrt(
      addVectors(entrywisemulVector(z.re, z.re), entrywisemulVector(z.im, z.im))
    );
  else if (z instanceof ComplexVector)
    return new Matrix(
      z.m,
      ...
      sqrt(
        addVectors(
          entrywisemulVector(z.re, z.re),
          entrywisemulVector(z.im, z.im)
        ),
        true
      )
    );
...
function ComplexMatrix(a, b, values, valuesimag) {
  ...
  if (arguments.length == 0) {
    ...
    if (typeof values == "undefined") {
      ...
      typeof valuesimag != "undefined" &&
      valuesimag instanceof Float64Array
    ) {
      this.re = values;
  ...
  } else error("Bad arguments to new ComplexMatrix()");
}
...
ComplexVector.prototype.setVectorScalar = function (rowsrange, B) {
  var i;
  for (i = 0; i < rowsrange.length; i++) A.set(rowsrange[i], B);
};
ComplexVector.prototype.setVectorVector = function (rowsrange, B) {
  var i;
  for (i = 0; i < rowsrange.length; i++) A.set(rowsrange[i], B[i]);
};

function real(z) {
  if (z instanceof Complex) return z.re;
  else if (z instanceof ComplexVector) return vectorCopy(z.re);
  else if (z instanceof ComplexMatrix) return new Matrix(z.m, z.n, z.re);
  else return copy(z);
}
function imag(z) {
  if (z instanceof Complex) return z.im;
  else if (z instanceof ComplexVector) return vectorCopy(z.im);
  else if (z instanceof ComplexMatrix) return new Matrix(z.m, z.n, z.im);
  else return 0;
}
...
function ifft(x) {
  ...
  for (var i = 0; i < n; i++) {
    X.re[i] /= n;
    X.im[i] /= n;
    if (Math.abs(X.im[i]) > 1e-6) isComplex = true;
  }
  if (isComplex) return X;
  else return X.re;
}
...
function dft(x) {
  ...
  else if (Math.log2(n) % 1 == 0) return fft(x);
  else {
...
plot(s)	
  ...
  const n = X.length;
  if (n == 1) return X.re[0];
  else if (Math.log2(n) % 1 == 0) return ifft(X);
  else {
...
function Diagram(canvasId) {
  if (typeof canvasId === "undefined") canvasId = "diagram";
...
Diagram.prototype.rect = function (x, y, w, h, color, txt, txtcolor, opacity) {
  if (typeof opacity === "undefined") var opacity = 0.6;
  if (typeof txtcolor === "undefined") var txtcolor = 0;
  if (typeof txt === "undefined") var txt = "";
  if (typeof color === "undefined") var color = 1;

  this.shapes.push(["rect", x, y, w, h, color, txt, txtcolor, opacity]);

  this.redraw();
};
Diagram.prototype.circle = function (
  x,
  y,
  w,
  h,
...
Diagram.prototype.image = function (
  x,
  y,
  w,
  h,
  ...
  for (n = 0; n < this.shapes.length; n++) {
    ...
    if (shape == "rect") {
      ...
      if (n == this.selectedShape || this.selectedShapes.indexOf(n) >= 0) {
        ...
        ctx.fillRect(
          (x - 0.005) * canvas.width,
          (y - 0.005) * canvas.height,
          (w + 0.01) * canvas.width,
          (h + 0.01) * canvas.height
        );
      }
      ...
      if (words.length == 1) {
        ...
        for (var i = 0; i < words.length; i++) {
          ctx.fillText(
            ...
            (x + w / 2) * canvas.width,
            (y + h / 2) * canvas.height -
              (words.length / 2 - i - 0.5) * (1.5 * txtsize)
          );
        }
      }
    }
...
Diagram.prototype.mouseselect = function (event) {
  ...
  if (
    Math.abs(x - this.mousexprev) >= 1 ||
    Math.abs(y - this.mouseyprev) >= 1
  ) {
    this.mousexprev = x;
    this.mouseyprev = y;
    ...
    while (i >= 0 && this.isInShape(x, y, this.shapes[i]) == false) i--;
...
Diagram.prototype.isInShape = function (x, y, shape) {
  ...
  if (shape[0] == "rect") {
    if (
      x > shape[1] * canvas.width &&
      x < (shape[1] + shape[3]) * canvas.width &&
      y > shape[2] * canvas.height &&
      y < (shape[2] + shape[4]) * canvas.height
    )
      return true;
    else return false;
...
Diagram.prototype.selectall = function (n) {
  for (var i = 0; i < this.shapes.length; i++) this.selectedShapes.push(i);
  this.redraw();
};
Diagram.prototype.deselectall = function () {
  while (this.selectedShapes.length > 0) this.selectedShapes.pop();
  this.redraw();
};
...
function Plot(canvasId) {
  if (typeof canvasId === "undefined") canvasId = "plotcanvas";
...
Plot.prototype.addPoint = function (x, y, color_idx, radius, opacity) {
  if (typeof color_idx === "undefined") color_idx = 0;
  if (typeof radius === "undefined") radius = 5;
  if (typeof opacity === "undefined") opacity = 1.1;
...
Plot.prototype.replot = function () {
  ...
  if (canvas.getContext) {
    ...
    for (var n = 0; n < this.areas.length; n++) {
      ...
      ctx.fillRect(
        (rectx - this.minX) * this.scaleX,
        canvas.height - (recty - this.minY) * this.scaleY,
        rectwidth * this.scaleX,
        rectheight * this.scaleY
      );
    }
    ...
    for (var n = 0; n < this.lines.length; n++) {
      ...
      if (
        (res[0] >= this.minX &&
          res[0] <= this.maxX &&
          res[1] >= this.minY &&
          res[1] <= this.maxY) || //start in plot
        (res[2] >= this.minX &&
          res[2] <= this.maxX &&
          res[3] >= this.minY &&
          res[3] <= this.maxY) || // end in plot
        (res[0] < this.minX &&
          res[2] > this.maxX &&
          ((res[1] >= this.minY && res[1] <= this.maxY) ||
            (res[3] >= this.minY && res[3] <= this.maxY))) || // overflow on x axis but y inside plot
        (res[2] < this.minX &&
          0 > this.maxX &&
          ((res[1] >= this.minY && res[1] <= this.maxY) ||
            (res[3] >= this.minY && res[3] <= this.maxY))) ||
        (res[1] < this.minY &&
          res[3] > this.maxY &&
          ((res[0] >= this.minX && res[0] <= this.maxY) ||
            (res[2] >= this.minX && res[2] <= this.maxX))) || // y-axis
        (res[3] < this.minY &&
          res[1] > this.maxY &&
          ((res[0] >= this.minX && res[0] <= this.maxX) ||
            (res[2] >= this.minX && res[2] <= this.maxX)))
      ) {
        x1 = xplot(res[0]);
        y1 = yplot(res[1]);
        x2 = xplot(res[2]);
        ...
        if (Math.abs(x2 - x1) > 1 || Math.abs(y2 - y1) > 1) {
          ...
          if (color_idx != cp) setcolor(ctx, color_idx);
      ...
      if (
        ...
        if (Math.abs(x - xp) > 1 || Math.abs(y - yp) > 1) {
          ...
          if (op != opacity || cp != color_idx) {
            if (opacity > 1.0) setcolor(ctx, color_idx);
            else setcolortransparent(ctx, color_idx, opacity);
          }
    ...
    for (var n = 0; n < this.paths.length; n++) {
      ...
      for (var i = 1; i < res[0].length; i++) {
        ...
        if (x >= 0 && x <= canvas.width && y >= 0 && y <= canvas.height) {
          if (marker) ctx.arc(x, y, radius, 0, 2 * Math.PI, true);
          ctx.lineTo(x, y);
        }
      }
      //ctx.closePath();
    ...
    for (var n = 0; n < this.fcts.length; n++) {
      ...
      if (fillareaTo !== false) {
        ...
        ctx.moveTo(
          canvas.width,
          canvas.height - (fillareaTo - this.minY) * this.scaleY
        );
      ...
      for (
        var x = this.minX;
        x < this.maxX;
        x += (this.maxX - this.minX) / this.Nsamples
      ) {
        var y = f(x);
        var yp = canvas.height - (y - this.minY) * this.scaleY;
        if (yp >= 0 && yp <= canvas.height) ctx.lineTo(xplot(x), yp);
        else ctx.moveTo(xplot(x), yp);
      }
      ctx.stroke();
      if (fillareaTo !== false) {
        ctx.closePath();
...
Plot.prototype.plot = function (f, legend, color_idx, dashed, fillareaTo) {
  if (typeof fillareaTo === "undefined") fillareaTo = false;

  if (typeof dashed === "undefined") dashed = false;
  if (typeof color_idx === "undefined") color_idx = -1;
...
Plot.prototype.plot_path = function (x, y, color_idx, radius, opacity, dashed) {
  if (typeof dashed === "undefined") var dashed = false;
  if (typeof color_idx === "undefined") var color_idx = -1;
  if (typeof opacity === "undefined") var opacity = 1;
  if (typeof radius === "undefined") var radius = 5;
...
Plot.prototype.translate = function (dx, dy) {
  ...
  if (
    newX >= 0 &&
    newX < this.buffer.width - canvas.width &&
    newY >= 0 &&
    newY < this.buffer.height - canvas.height
  ) {
    this.viewX = newX;
    this.viewY = newY;
...
Plot.prototype.zoom = function (zx, zy, x, y) {
  ...
  if (zy > 0) canvas.height *= zy;
  else canvas.height = viewcanvas.height;
  if (zx > 0) canvas.width *= zx;
  else canvas.width = viewcanvas.width;

  // do not zoom out further than original
  if (canvas.width < viewcanvas.width) canvas.width = viewcanvas.width;
  if (canvas.height < viewcanvas.height) canvas.height = viewcanvas.height;
  ...
  if (sx != this.scaleX || sy != this.scaleY) this.replot();
};
...
Plot.prototype.pick_point = function (e) {
  if (e.button == 0) {
    ...
    return [x, y];
  } else return false; // not correct button
};

Plot.prototype.proximityX = function (x, x0, epsilon) {
  if (typeof epsilon === "undefined") epsilon = (this.maxX - this.minX) / 20;
  ...
  if (canvas.getContext) {
    ...
    for (var i = 0; i < this.x.length; i++) {
      if (
        ...
        if (z >= this.cmap.length) z = this.cmap.length - 1;
        if (z < 0) z = 0;
...
function Plot2D(canvasId, tableId) {
  if (typeof canvasId === "undefined") this.canvasId = "plotcanvas2D";
  else this.canvasId = canvasId;

  if (typeof tableId === "undefined") this.tableId = "";
  // No data table by default
...
Plot2D.prototype.clearData = function () {
  if (this.tableId != "")
    document.getElementById(this.tableId).innerHTML =
      "<tr> <td> x1 </td><td> x2 </td><td> y </td></tr> ";
...
Plot2D.prototype.levelcurve = function (f, level) {
  ...
  if (canvas.getContext) {
    ...
    for (
      var x1 = this.minX1;
      x1 < this.maxX1;
      x1 += (this.maxX1 - this.minX1) / this.NsamplesX1
    ) {
      Y[i] = new Array();
      for (
        var x2 = this.minX2;
        x2 < this.maxX2;
        x2 += (this.maxX2 - this.minX2) / this.NsamplesX2
      ) {
        var x = [x1, x2];
        Y[i][j] = f(x);
        j++;
      ...
      for (
        ...
        if (
          ...
          /*
					...
					*/
          ctx.fillRect(
            (x1 - this.minX1) * this.scaleX1 - 2,
            canvas.height - (x2 - this.minX2) * this.scaleX2 - 2,
            4,
            4
          );
        }
    ...
    for (
      ...
      for (
        ...
        if (Math.abs(Y[i][j]) < 0.00001) {
          ...
          ctx.fillStyle =
            "rgba(0,0," +
            (255 - Math.floor((Y[i][j] - minf) * colorScale)) +
            ", 0.9)";
        } else {
          //ctx.fillStyle = "rgba(" + Math.floor((Y[i][j] - minf) * colorScale ) + ",0,255,0.5)";
          ctx.fillStyle =
            "rgba(" + Math.floor((Y[i][j] - minf) * colorScale) + ",0,0, 0.9)";
        }
        ctx.fillRect(
          (x1 - this.minX1) * this.scaleX1 - 2,
          canvas.height - (x2 - this.minX2) * this.scaleX2 - 2,
...
Plot2D.prototype.point = function (x1, x2, color_idx, opacity, radius) {
  ...
  if (canvas.getContext) {
    ...
    if (opacity < 1.0) setcolortransparent(ctx, color_idx, opacity);
    else setcolor(ctx, color_idx);

    ctx.beginPath();
    ctx.arc(
      (x1 - this.minX1) * this.scaleX1,
      canvas.height - (x2 - this.minX2) * this.scaleX2,
      radius,
      0,
      2 * Math.PI,
      true
...
Plot2D.prototype.plot_data = function () {
  if (this.m != this.Yapp.length) this.m = this.Yapp.length;
...
Plot2D.prototype.plot_vector = function (
  start_x1,
  start_x2,
  end_x1,
  end_x2,
...
Plot2D.prototype.plot_line = function (
  start_x1,
  start_x2,
  end_x1,
  end_x2,
  ...
  if (canvas.getContext) {
    ...
    if (typeof linename !== "undefined") {
      if (linename != "") {
        ...
        ctx.strokeText(
          linename,
          (end_x1 + start_x1) / 2 - 10,
          canvas.height - 10 - (end_x2 + start_x2) / 2
        );
      }
    }
  }
...
Plot2D.prototype.plot_classifier = function (w, b, coloridx, disappear) {
  if (typeof disappear === "undefined") var disappear = false;
  if (typeof coloridx === "undefined") var coloridx = 0;
  ...
  if (canvas.getContext) {
    ...
    ctx.lineWidth = "3";
    if (disappear) ctx.strokeStyle = "grey";
    else setcolor(ctx, coloridx);

    ctx.setLineDash([1, 0]);
    ctx.beginPath();
    ctx.moveTo(
      (x1 - this.minX1) * this.scaleX1,
      canvas.height / 2 - y1 * this.scaleX2
    );
    ctx.lineTo(
      (x2 - this.minX1) * this.scaleX1,
      canvas.height / 2 - y2 * this.scaleX2
    );
    ctx.stroke();
  }
};
...
Plot2D.prototype.plotmathjax = function (stringindex, x, y) {
  ...
  if (canvas.getContext) {
    ...
    var label = document.getElementById("jaxstring" + stringindex);
    label.style.top =
      canvas.height - (y - this.minX2) * this.scaleX2 + canvas.offsetTop;
...
function Plot3D(canvasId) {
  if (typeof canvasId === "undefined") this.canvasId = "plotcanvas3D";
  else this.canvasId = canvasId;
...
Plot3D.prototype.replot = function () {
  ...
  for (i = 0; i < this.Y.length; i++) {
    ...
    if (distance < maxDistance) opacity = distance / maxDistance;
    else opacity = 1.0;
...
Plot3D.prototype.rotateZ = function (deltaangle, do_replot) {
  if (typeof do_replot == "undefined") var do_replot = true;

  this.angleZ += deltaangle;
  if (do_replot) this.replot();
};
...
Plot3D.prototype.mouserotation = function (e) {
  if (this.ROTATING) {
    ...
    if (Math.abs(dx) > 0.2) this.rotateZ(dx / 20, !(Math.abs(dy) > 0.2));
    if (Math.abs(dy) > 0.2) this.rotateX(dy / 20);
  }
};
...
Plot3D.prototype.mousezoom = function (e) {
  ...
  } else if (plot3.cameraDistance > 100) plot3.cameraDistance = 100;
...
Plot3D.prototype.line = function (
  start,
  end,
  linename,
  linecolor,
...
Plot3D.prototype.plot_line = function (start, end, linename, color) {
  if (typeof color === "undefined") var color = 0;
...
Plot3D.prototype.plane = function (start, end, polyname, color) {
  if (typeof color === "undefined") var color = 3;
  if (typeof polyname === "undefined") var polyname = "";
...
Plot3D.prototype.drawplane = function (start, end, polyname, color) {
  ...
  this.polygone2D(
    [start_x1, corner1_x1, end_x1, corner2_x1],
    [start_x2, corner1_x2, end_x2, corner2_x2],
    polyname,
    color
  );
};
...
Plot3D.prototype.sphere = function (center, radius, spherename, color) {
  if (typeof color === "undefined") var color = 1;
  if (typeof spherename === "undefined") var spherename = "";
...
Plot3D.prototype.drawsphere = function (center, radius, spherename, color) {
  ...
  if (distance >= 0) {
    ...
    if (distance < maxDistance) opacity = 0.5 * (distance / maxDistance);
...
Plot3D.prototype.point2D = function (x1, x2, color_idx, opacity, radius) {
  if (
    ...
    if (typeof opacity === "undefined") var opacity = 1.1;
    if (typeof radius === "undefined") var radius = 5;
    ...
    if (canvas.getContext) {
      ...
      ctx.beginPath();
      ctx.arc(
        (x1 - this.viewminX1) * this.viewscaleX1,
        canvas.height - (x2 - this.viewminX2) * this.viewscaleX2,
        radius,
        0,
        2 * Math.PI,
        true
...
Plot3D.prototype.line2D = function (
  start_x1,
  start_x2,
  end_x1,
  end_x2,
  ...
  if (start_x1 < 0) start_x1 = 0;
  if (start_x1 >= canvas.width) start_x1 = canvas.width - 1;
  if (start_x2 <= 0) start_x2 = 1;
  if (start_x2 > canvas.height) start_x2 = canvas.height;
  if (end_x1 < 0) end_x1 = 0;
  if (end_x1 >= canvas.width) end_x1 = canvas.width - 1;
  if (end_x2 <= 0) end_x2 = 1;
  if (end_x2 > canvas.height) start_x2 = canvas.height;
...
Plot3D.prototype.disk2D = function (
  x1,
  x2,
  radius,
  spherename,
  ...
  if (
    x1 + radius >= this.viewminX1 &&
    x1 - radius <= this.viewmaxX1 &&
    x2 + radius >= this.viewminX2 &&
    x2 - radius <= this.viewmaxX2
  ) {
    var canvas = document.getElementById(this.canvasId);
    if (canvas.getContext) {
      var ctx = canvas.getContext("2d");
      ...
      if (opacity < 1.0)
        setcolortransparentgradient(
          ctx,
          color,
          opacity,
          Math.sqrt(x1view * x1view + x2view * x2view) + radius
        );
      else
        setcolorgradient(
          ctx,
            ...
            if (words.length == 1) {
              ...
              for (var i = 0; i < words.length; i++) {
                ctx.fillText(
                  words[i],
                  x1view,
                  x2view - (words.length / 2 - i - 0.5) * (1.5 * txtsize)
                );
              }
            }
            ctx.font = tmpfont;
...
Plot3D.prototype.animate = function (samplingRate) {
  if (typeof samplingRate === "undefined") var samplingRate = this.animateAuto;
...
Plot3D.prototype.isInSphere = function (x, y, z, sphere) {
  ...
  if (norm2 <= sphere[1] * sphere[1]) return true;
  else return false;
};
...
function setcolorgradient(ctx, color_idx, size) {
  if (typeof size === "undefined") var size = 400 * Math.sqrt(2);
